...
public class UDPGameServer extends UDPServer{
    ...
    public void respond(DatagramSocket socket, DatagramPacket packet) throws IOException {
        ...
        int index = Integer.parseInt(sSplit[1]); // indice della prossima parola che si vuole tradurre
        String trad = null;
        ...
        } else if (index == -1) { // ultimo messaggio che invia il tempo
            //System.out.println(currentPlayer + " ha impiegato " + trad + " millisec!");
        }
        ...
        if (index < words.length) { // invio della parola successiva da tradurre
            ...
            if (currentPlayer.equals(this.p1ID)) {
                this.p1Finito = true;
            } else {
                this.p2Finito = true;
            }
